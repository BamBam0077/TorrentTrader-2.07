...
if (!ini_get("register_globals")) {
	if (@count($_GET)) extract($_GET, EXTR_SKIP);
	if (@count($_POST)) extract($_POST, EXTR_SKIP);
	if (@count($_COOKIE)) extract($_COOKIE, EXTR_SKIP);
	if (@count($_FILES)) extract($_FILES, EXTR_SKIP);
}
...
function dbconn($autoclean = false) {
	...
	// Not logged in or chosen theme/language has been deleted.
	if (!$THEME) {
...
function userlogin() {
	...
	if (is_ipv6($ip))
		...
		$nip = ip2long($ip);
	$res = mysql_query("SELECT * FROM bans")  or die(mysql_error());
	...
	$res = mysql_query("SELECT * FROM users INNER JOIN groups ON users.class=groups.group_id WHERE id=$_COOKIE[uid] AND users.enabled='yes' AND users.status = 'confirmed'") or die(mysql_error());
	$row = mysql_fetch_array($res);
...
function stdhead($title = "") {
	...
	if (!$site_config["SITE_ONLINE"]){
		if ($CURUSER["control_panel"] != "yes") {
			...
			echo '<BR><BR><BR><CENTER><B><FONT COLOR=RED>SITE OFFLINE, STAFF ONLY VIEWING! DO NOT LOGOUT</FONT></B><BR>If you logout please edit backend/config.php and set SITE_ONLINE to true </CENTER><BR><BR>';
		}
	}
	//end check
...
function deletetorrent($id) {
	...
	foreach(explode(".","peers.comments.ratings.files.announce.completed") as $x)
...
function torrenttable($res) {
	...
	foreach ($cols as $col) {
		switch ($col) {
			case 'category':
				echo "<td class=ttable_head>".T_("TYPE")."</td>";
			break;
			case 'name':
				echo "<td class=ttable_head>".T_("NAME")."</td>";
			break;
			case 'dl':
			...
			break;
			case 'size':
				echo "<td class=ttable_head>".T_("SIZE")."</td>";
			break;
			case 'completed':
			...
			break;
			case 'health':
				echo "<td class=ttable_head>".T_("HEALTH")."</td>";
			break;
			case 'external':
			...
			break;
			case 'added':
				echo "<td class=ttable_head>".T_("ADDED")."</td>";
			break;
			case 'speed':
				echo "<td class=ttable_head>".T_("SPEED")."</td>";
			break;
			case 'wait':
				if ($wait)
			...
			break;
			case 'rating':
				echo "<td class=ttable_head>".T_("RATINGS")."</td>";
			break;
		}
	}
	...
	foreach ($cols as $col) {
		switch ($col) {
			...
			break;
			case 'leechers':
				print("<td class=ttable_col$x align=center><font color=red><b>" . number_format($row["leechers"]) . "</b></font></td>\n");
			break;
			case 'health':
		...
		if (count($expandrows)) {
			...
			foreach ($expandrows as $expandrow) {
				switch ($expandrow) {
					...
					break;
					case 'completed':
						print("<tr><td><b>".T_("COMPLETED")."</B>: ".$row['times_completed']."</td></tr>");
					break;
				}
			}
...
function commenttable($res) {
	...
	while ($row = mysql_fetch_assoc($res)) {
		...
		if($CURUSER["edit_torrents"]=="yes" || $CURUSER["edit_forum"]=="yes" || $CURUSER['id'] == $row['user']){
			print("[<a href=comments.php?id=" . $row["id"] . "&type=torrent&edit=1>".T_("EDIT")."</a>]&nbsp;");
		}
		if($CURUSER["delete_torrents"]=="yes" || $CURUSER["delete_forum"]=="yes"){
			print("[<a href=comments.php?id=" . $row["id"] . "&type=torrent&delete=1>".T_("DEL")."</a>]&nbsp;");
		}

		print("[<a href=report.php?comment=" . $row["id"] . ">".T_("REPORT")."</a>]&nbsp;");

		print("".T_("POSTED").": ".date("d-m-Y \\a\\t H:i:s", utc_to_tz_time($row['added'])));
		print("</td></tr>");

		if ($privacylevel != "strong" || ($CURUSER["control_panel"] == "yes")) {
			print("<tr><td valign=top width=150 align=left class=table_col2><center><b>$postername</b><br><i>$title</i></center><br>".T_("UPLOADED").": $useruploaded<br>".T_("DOWNLOADED").": $userdownloaded<br>".T_("RATIO").": $userratio<br><br><center><img width=80 height=80 src=$avatar></center><br></td>");
		}else{

...
<HTML>
<HEAD>
<TITLE><?php echo $site_config['SITENAME']?><?php echo T_("SHOUTBOX")?></TITLE>
...
<?php
	...
	stdhead();
	begin_frame(T_("SHOUTBOX") . " History");
...
if(!isset($_GET['history'])) {
	if (isset($_COOKIE["pass"])){
		...
		echo "</td>";
		echo "<td>";
		echo "<a href=".$site_config['SITEURL']."/backend/smilies.php?action=display target=_blank><small>".T_("SMILIES")."</small></a>";
		echo " - <a href=".$site_config['SITEURL']."/tags.php target=_blank><small>".T_("TAGS")."</small></a>";
		echo "<br>";
		echo "<a href='shoutbox.php'><small>".T_("REFRESH")."</small></a>";
		echo " - <a href='".$site_config['SITEURL']."/shoutbox.php?history=1' target=_blank><small>".T_("HISTORY")."</small></a>";
		echo "</td>";
		echo "</tr>";
		echo "</table></CENTER>";
		echo "</form>";

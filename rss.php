...
if (isset($_GET["custom"])){
	...
	<tr>
		...
		<td class="table_col2">
			<input type="radio" name="dllink" value="0" checked> Details link<br/>
	...
	<tr>
		...
		<td class="table_col2">
			<input type="radio" name="cookies" value="1" checked> Standard (cookies)<br/>
	...
	<li>Our feeds display only the latest 50 uploaded Torrents as default.</li><BR>
	</ul>
	</div>
	<?php
	end_frame();
...
$res = mysql_query("SELECT torrents.id, torrents.name, torrents.size, torrents.category, torrents.added, torrents.leechers, torrents.seeders, categories.parent_cat as cat_parent, categories.name AS cat_name FROM torrents LEFT JOIN categories ON category = categories.id $where ORDER BY added DESC $limit");
